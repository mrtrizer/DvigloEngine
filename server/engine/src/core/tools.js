...
//----------------------------------------------------------------------
export var ImgLoader = function (onLoaded) {
	...
	{
		this.resCounter++;
		if (this.resCounter >= this.resLoadCount)
			this.onLoaded();
	}
	...
	this.getImage = function (imgPath)
	{
		return this.images[imgPath];
	}
	...
	this.loadImage = function (imgPath)
	{
		...
		var img = new Image();
		img.src = imgPath;
		img.onload = function () {object.imageLoaded();}
...
{
	constructor(host,userId,authKey,apiId)
	{
		this.host = host;
		this.userId = userId;
	...
	sendRequest (funcName,args,method,parseFunction,procError)
	{
		var xmlhttp;
...
export function findItemN(array,func)
{
	for (var i in array)
	{
		if (func(array[i]))
...
//----------------------------------------------------------------------
//Math tools namespace
//----------------------------------------------------------------------
export var MathTools = {};

MathTools.getIterPoint = function (p1, p2, p3, p4)
{
...
MathTools.getRandomInt = function (min, max)
{
	return Math.floor(Math.random() * (max - min + 1)) + min;
}


MathTools.getLineIterPoint = function (p1, p2, line, tileWidth, tileHeight)
{
	return MathTools.getIterPoint(p1,p2,{x:line.x1 * tileWidth, y:line.y1 * tileHeight}, {x:line.x2 * tileWidth, y:line.y2 * tileHeight});
}
...
MathTools.isInRect = function(pos, rect)
{
	if ((pos.x > rect.x) && (pos.y > rect.y) && (pos.x < rect.width + rect.x) && (pos.y < rect.height + rect.y))
		return true;
}

MathTools.isInRectAbs = function(pos, rect)
{
	if ((pos.x > rect.x1) && (pos.y > rect.y1) && (pos.x < rect.x2) && (pos.y < rect.y2))
		return true;
}

MathTools.getDist = function(from, to)
{
	var dX = from.x - to.x;
	var dY = from.y - to.y;
